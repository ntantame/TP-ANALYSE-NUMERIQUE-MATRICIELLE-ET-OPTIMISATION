...
def methode_jacobi(A, b, x0, epsilon, kmax):
...
# Vérification : la matrice A doit avoir une diagonale non nulle
diag_A = np.diag(A)
if np.any(np.abs(diag_A) < 1e-10):
    print("ERREUR : La matrice A a des éléments diagonaux nuls!")
    print("La méthode de Jacobi ne peut pas être appliquée.")
else:
    # Résolution
    x = methode_jacobi(A, b, x0, epsilon, kmax)
